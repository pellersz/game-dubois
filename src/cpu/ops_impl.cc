...
void Cpu::op_0x08() 
{
    ...
    writtenToMemory(offs);
    writtenToMemory(offs + 1);
} 
...
{ 
    unsigned short addr = getHL();
    opInc(memory[addr], memory[addr + 1]);
    writtenToMemory(addr);
    writtenToMemory(addr + 1);
} 
